...
def spacer(length=None, background=None, name=None):
    ...
    return widget.Spacer(length=length, background=background, name=name)
...
def powerline(
    direction, background=None, foreground=None, name=None, mouse_callbacks=None
):
    ...
    return widget.TextBox(
        text=LEFT_POWERLINE if direction == "l" else RIGHT_POWERLINE,
...
def toggle_picom():
    ...
    output = int(
        subprocess.check_output(
            os.path.expanduser("~/.shell-scripts/qtile/toggle-picom.sh")
        )
    )
    PICOM_ON = output == 1
...
def toggle_conky():
    ...
    qtile.cmd_spawn(
        os.path.expanduser("~/.shell-scripts/conky/launch-all.sh")
    ) if return_code == 1 else subprocess.run(["killall", "conky"])
...
keys = [
    ...
    Key([mod], "0", toggle_info(), desc="Toggle Info"),
    ...
    Key(
        [mod, "control"], "l", lazy.layout.swap_right(), desc="Move window to the right"
    ),
    ...
    Key([mod, "shift"], "r", lazy.layout.normalize(), desc="Reset all window sizes"),
    Key(
        [mod, "control"],
        "s",
    ...
    Screen(
        top=bar.Bar(
            widgets=[
                ...
                else separator(length=4, name="groupbox_separator"),
                ...
                if POWERLINE_ENABLED
                else separator(length=4, name="widgetbox_separator"),
                widget.WidgetBox(
                    widgets=[
                        widget.CPU(
                        ...
                        if POWERLINE_ENABLED
                        else separator(length=4, name="net_separator"),
                        widget.Net(
                            interface="wlan0",
                            format="\uf1eb  {down} \uf175\uf176 {up}",
                        ...
                        if POWERLINE_ENABLED
                        else separator(length=4, name="backlight_separator"),
                        # widget.Bluetooth(),
                        widget.Backlight(
                ...
                if POWERLINE_ENABLED
                else separator(length=4, name="battery_separator"),
                widget.Battery(
                ...
                widget.WidgetBox(
                    widgets=[
                        ...
                        widget.TextBox(
                            text=TOGGLE_ON if PICOM_ON else TOGGLE_OFF,
                            fontsize=30,
                            width=45,
                            padding=10,
                            background=COLORS["cyan"][0] if POWERLINE_ENABLED else None,
